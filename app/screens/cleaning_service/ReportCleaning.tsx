...
export default function ReportCleaning({ navigation }: any) {
  ...
  useEffect(() => {
    const verifyAttendance = async () => {
      ...
      try {
        ...
        const res = await getMyAttendanceReq({ startDate: today, endDate: today });
        const sudahMasuk = (res.data || []).some((a: any) => a.type === "masuk");
      ...
      (async () => {
        try {
          ...
          if (reverse.length > 0) {
            ...
            setAlamatRealtime(`${street || name || ""}, ${district || ""}, ${city || ""}`);
          }
        } catch (e) {
  ...
  const handleSubmit = async () => {
    if (!hasCheckedInToday) return showToast("Absen dulu baru laporan, Bre!", "error");
    if (!description || description.length < 5) return showToast("Kasih deskripsi yang jelas (min 5 huru)!", "error");
    if (photos.length === 0) return showToast("Mana foto buktinya? No hoax ya!", "error");
    ...
    try {
      ...
      showToast(error.response?.data?.message || "Gagal kirim laporan", "error");
    } finally {
            ...
            <View>
              ...
              <View className="bg-black/70 p-5 rounded-[24px] border border-white/20 mb-10">
                <View className="flex-row items-center mb-1">
                  <MaterialCommunityIcons name="map-marker-radius" size={18} color="#0099ff" />
                  <Typography className="text-white font-bold ml-2 text-xs" numberOfLines={1}>{alamatRealtime}</Typography>
                </View>
                <Typography className="text-white/50 text-[10px]">📍 {lokasi?.lat.toFixed(6)}, {lokasi?.lng.toFixed(6)}</Typography>
              </View>

              {/* Shutter Button */}
              <View className="flex-row items-center justify-center gap-16 mb-6">
                <TouchableOpacity onPress={() => setIsCameraOpen(false)}>
                  <MaterialCommunityIcons name="close-circle-outline" size={40} color="white" />
                </TouchableOpacity>
                ...
                >
                  {isUploading ? <ActivityIndicator color="white" /> : <View className="w-16 h-16 rounded-full bg-white" />}
                </TouchableOpacity>
  ...
  return (
    <Container className="bg-background dark:bg-slate-950">
      ...
      <Header title="Laporan Cleaning" showBack onBack={() => navigation.goBack()} />

      <ScrollView className="px-5 py-4" showsVerticalScrollIndicator={false}>
        <Card className="p-6 mb-8 bg-card dark:bg-slate-900 border-border rounded-[32px] shadow-2xl">
          <Typography variant="h2" className="mb-2 dark:text-white font-black text-primary">Ada Temuan Apa, Bre?</Typography>
          <Typography className="text-gray-500 mb-6 text-xs">Laporkan fasilitas kotor atau rusak biar langsung ditindak!</Typography>
          ...
          <Typography variant="h3" className="mb-4 dark:text-white font-bold">Bukti Foto Laporan</Typography>
          
          <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row mb-6 py-3">
            {photos.map((url, index) => (
              <View key={index} className="mr-4 shadow-xl">
                <Image source={{ uri: url }} className="w-40 h-56 rounded-[24px]" />
                <TouchableOpacity 
            ...
            {photos.length < 5 && (
              <TouchableOpacity 
                onPress={() => { ambilLokasi(); setIsCameraOpen(true); }}
              ...
              >
                ...
                <Typography className="text-primary font-black mt-3">Ambil Foto</Typography>
              </TouchableOpacity>
            )}
          </ScrollView>
          ...
          <View className="flex-row items-center p-3">
             <MaterialCommunityIcons name="shield-check" size={20} color="#0099ff" />
             <Typography className="text-gray-500 text-[10px] ml-2 flex-1">Laporan lu bakal masuk ke sistem admin dengan status 'Open'[cite: 10]</Typography>
          </View>
        </Card>

        <Button 
          title={hasCheckedInToday ? "KIRIM LAPORAN SEKARANG 🔥" : "ABSEN DULU BARU LAPOR 🔒"} 

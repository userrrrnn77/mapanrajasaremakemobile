...
export const useLokasi = () => {
  ...
  const ambilLokasi = async (): Promise<Koordinat | null> => {
    ...
    try {
      ...
      if (location.mocked) {
        Alert.alert("Curang!", "Fake GPS terdeteksi oleh sistem");
        return null;
      }
      ...
      Alert.alert("Error", "Gagal ambil lokasi. Pastikan GPS nyala.");
      return null;
    } finally {
  ...
  /**
   * 📏 Rumus Haversine buat cek jarak meter
   */
  const hitungJarakSederhana = (
    ...
    return R * c; // Balikin jarak dalam meter
  };
